...
完整功能与 UI 规范见 [`docs/SPEC.md`](docs/SPEC.md)，版本更新历史见
...
OpenWire 带有一个提权引擎，因此我们认真对待信任边界：

- **鉴权的 IPC** — 提权引擎从操作系统的管道句柄取得每个客户端的身份，并绑定到它亲手
  启动的那个 GUI 进程；管道 ACL 仅限当前用户。跨用户连接与管道抢注一律拒绝。
- **登录最小权限** — GUI 以普通的按用户自启项启动，绝不注册任何提权自启动任务。
- **你的防火墙规则归你** — OpenWire 只管理带有自身标记的规则、原子替换，绝不修改或删除
  你已有的防火墙规则。锁定 / 全拦始终是手动且可逆的。
- **数据只留在本地且受锁** — SQLite 历史处于收紧的 DACL 之下；VirusTotal 密钥用 DPAPI
  加密落盘；崩溃日志私有且轮转。无账号、无云端、无遥测。

完整的加固历史见 [`CHANGELOG.md`](CHANGELOG.md)。
