...
![流量圖](docs/screenshots/traffic-pencil.png)
...
| 世界地圖 | 3D 地球 |
|---|---|
| ![2D 世界地圖](docs/screenshots/map.png) | ![3D 地球](docs/screenshots/globe.png) |

| 硬體資源 | 設定 |
|---|---|
| ![硬體](docs/screenshots/hardware.png) | ![設定](docs/screenshots/settings.png) |
...
完整功能與 UI 規範見 [`docs/SPEC.md`](docs/SPEC.md)，版本更新歷史見
...
OpenWire 帶有一個提權引擎，因此我們認真對待信任邊界：

- **鑑權的 IPC** — 提權引擎從作業系統的管道控制代碼取得每個用戶端的身分，並繫結到它
  親手啟動的那個 GUI 行程；管道 ACL 僅限目前使用者。跨使用者連線與管道搶註一律拒絕。
- **登入最小權限** — GUI 以一般的按使用者自啟項啟動，絕不註冊任何提權自動啟動工作。
- **你的防火牆規則歸你** — OpenWire 只管理帶有自身標記的規則、原子替換，絕不修改或刪除
  你既有的防火牆規則。鎖定 / 全擋始終是手動且可逆的。
- **資料只留在本機且受鎖** — SQLite 歷史處於收緊的 DACL 之下；VirusTotal 金鑰以 DPAPI
  加密落盤；當機日誌私有且輪替。無帳號、無雲端、無遙測。

完整的加固歷史見 [`CHANGELOG.md`](CHANGELOG.md)。
